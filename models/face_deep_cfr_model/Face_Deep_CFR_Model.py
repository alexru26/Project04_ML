...
class FaceDeepCFRModel:
    """
    Implementation of Deep CFR algorithm which takes into account bluffing
    """
    ...
    def __init__(self, env, model_path, lr=0.001):
        ...
        input_dim = self.env.state_shape[0][0] + 1  # State feature size
    ...
    def traverse_tree(self, probs, player_id):
        ...
        state_utility = np.zeros(
            self.env.num_players)  # Utility for this state for both players which takes into action probability (weighted average)
    ...
    def action_probs(self, obs, legal_actions):
        ...
        if len(obs) == 36:
            bluff = 0  # TODO: Get bluff from trained model
            obs = np.append(obs, bluff)
    ...
    def get_state(self, player_id):
        ...
        bluff = 0  # TODO: Get bluff from trained model
        state['obs'] = np.append(state['obs'], bluff)
